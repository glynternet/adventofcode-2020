...
main =
    AdventOfCode.day
        ...
        , testInput = inputTest
        , part1 = part1
        , part1TestExpected = Just "11"
        , part1Expected = Just "7128"
        , part2 = part2
        , part2Expected = Just "3640"
        , debugWindows = \_ -> []
        }
...
part1 =
    List.map (stringAnswers >> Set.size)
        >> List.sum
        >> String.fromInt


stringAnswers : String -> Set.Set Char
stringAnswers =
    String.toList >> List.filter Char.isAlpha >> Set.fromList


part2 : List String -> String
part2 =
    List.map (String.split "\n")
        >> List.map (\group -> ( List.length group, group ))
        >> List.map
            (\( groupSize, listOfGroupAnswers ) ->
                listOfGroupAnswers
                    |> List.map (stringAnswers >> Set.toList)
                    |> List.concat
                    |> MultiDict.fromList Char.toCode
                    |> matchingAnswersCount groupSize
            )
        >> List.sum
        >> String.fromInt


matchingAnswersCount : Int -> Dict.Dict Int (List Char) -> Int
matchingAnswersCount expectedSize answers =
    Dict.values answers
        |> List.filter (\aas -> aas |> List.length |> (==) expectedSize)
        |> List.length
